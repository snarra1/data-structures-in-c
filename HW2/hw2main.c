...
void getMinFans(int fsize[],int nfan,int cooling)
{
	...
	int sum=0,min=9999,addn=0,len=0,temps=0;
	int temp[nfan],finalarr[nfan];
	for(i=0;i<nfan-1;i++)
    	{
		for(j=i+1;j<nfan;j++)
		{
			if(fsize[i]==fsize[j]) // checks if any data is repeated
			{
				printf("\n Size %d is repeated at position %d\n",fsize[i],j+1);
				return;
			}
			if(fsize[i]>fsize[j]) //sorts the array in ascending order
			{
				temps=fsize[i];	//swaps the elements
				fsize[i]=fsize[j];
				fsize[j]=temps;
			}
		}
    ...
    for(i=0;i<nfan;i++)
	{
		...
		{
            if(i!=j)//same size should not be repeated
            {
...
int main()
{
	...
	int numFans=4, coolingNum=5, usedSize=4, totalSize=4, minhd=0, arraySize=3, pos=0;
	int fanSizes[]={1,2,3,4}, fSizes[]={1,2,3,4,5,6,10};
	int used[]={14,20,10,22}, total[]={80,120,120,80}, used1[]={60,20,30,40};
	int cost[]={4,3,7}, sales[]={5,6,9}, unitSale[]={1,1,1};
	int cost1[]={1,2,7}, sales1[]={2,6,9}, unitSale1[]={1,2,6};

	getMinFans(fanSizes,numFans,coolingNum); //calls getMinFans function with 4 fan sizes of cooling 5
	numFans=7;
	coolingNum=10;
	getMinFans(fSizes,numFans,coolingNum); //calling getMinFans function for 7 fan sizes and cooling 10
	minhd=minHardDrives(used,usedSize,total,totalSize); //calls minHardDrives function for input 'used'
	printf("\n %d\n",minhd); //prints minimum hard drives required
	minhd=minHardDrives(used1,usedSize,total,totalSize); //calls minHardDrives function for input 'used1'
	printf("\n %d\n",minhd);
	pos=findProfit(cost,sales,unitSale,arraySize);
	printf("\n %d \n",pos);
	pos=findProfit(cost1,sales1,unitSale1,arraySize);
	printf("\n %d \n",pos);
	printf("\n ");
